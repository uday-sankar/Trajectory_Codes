...
def step_Euler_mod(xi,yi,vxi,vyi,axi,ayi,dt):#modified Euler, acombination of RK4 and Verlet 
    ...
    my = mu*x_half**2
    I = my*Ich/(my+Ich)
    ax_half = Fx_half/mu + x_half*(I*vy_half/my)**2
    ay_half = Fy_half/I - 2*I/my*vx_half*vy_half/x_half
    ...
    my = mu*xn**2
    I = my*Ich/(my+Ich)
    axn = Fxn/mu + xn*(vyn*I/my)**2
    ayn = Fyn/I - 2*I/my*vxn*vyn/xn

    #Step 3[Final calculation]

    ax_avg = (axi + 2*ax_half + axn)/4
    ay_avg = (ayi + 2*ay_half + ayn)/4

    vxf = vxi + ax_avg*dt
    vyf = vyi + ay_avg*dt


    return xn, yn, vxf, vyf , axn, ayn
...
print("Time Taken for excecution:",end_t-start_t)
print("Done")
