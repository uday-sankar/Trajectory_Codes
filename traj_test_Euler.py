...
def step(xi,yi,vxi,vyi,axi=0,ayi=0,dt=0.01):#the inital cordinateds for each step Velocity verlet
    ...
    Fxn, Fyn = F(xn,yn) #new accelerations (at n+1 step) 
    axn = Fxn/mx 
    ayn = Fyn/my
...
def step_Euler_mod(xi,yi,vxi,vyi,axi,ayi,dt):#modified Euler, acombination of RK4 and Verlet 
    ...
    my = mu*x_half**2
    ax_half = Fx_half/mx + x_half*vy_half**2
...
def run_Euler(xinit,yinit,pxinit,pyinit,Steps=1000,dt=0.01):
    Fxinit,Fyinit = F(xinit,yinit)
    my = mu*xinit**2
    vxt, vyt = pxinit/mx, pyinit/my
    axt = Fxinit/mx + xinit*vyt**2
    ...
    for i in range(Steps):
        ...
        Px.append(vxt*mx)
        Py.append(vyt*myn)
        e = PES(x_new,y_new) + 0.5*mx*vxt**2 + 0.5*myn*vyt**2
        E.append(e)
    return xt,yt,Px,Py,E
...
print("Euler Method")
X,Y,Px,Py,E=run_Euler(xinit,yinit,pxinit,pyinit,Steps,dt)
...
X,Y,Px,Py,E=run_Euler(xl,yl,-pxl,-pyl,Steps,dt)
